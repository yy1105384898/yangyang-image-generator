...
![科研图工作台](docs/screenshots/research-workbench.png)
...
- 科研图工作台：根据页面里粘贴的论文摘要、实验步骤或课题材料，生成中文科研绘图提示词、Mermaid 流程图和可下载的科研图输出。
...
科研图工作台位于 `#research`，面向论文配图、实验流程图、机制流程图和图形摘要草图。它不读取或分析当前代码仓库，只使用页面中填写或粘贴的科研材料，包括论文摘要、实验步骤、课题说明、模型结构、输入输出逻辑和实验条件。

主要流程：

1. 在左侧填写“绘图目标”和“论文摘要 / 项目内容”。如果没有填写科研材料，系统会提示先补充内容。
2. 选择图类型和视觉风格后，点击“生成科研绘图提示词”。文本模型会按中文主体、构图、结构、风格生成可直接用于生图的提示词。
3. 点击“生成流程图”可生成 Mermaid 流程图源码，并在画布中预览。流程图可以导出为 SVG 或 PNG。
4. 画布中的“文生图节点”使用和商业生图工作台一致的生图模型与 Key 池；点击节点里的“启动文生图”只生成该节点对应的一张图。
5. 顶部“生成科研图”会按画布里的文生图节点数量批量提交任务。
6. 生成完成后，科研图会回填到对应输出节点，节点图片右下角提供“下载”按钮。
7. “保存工程”保存当前画布节点、连接关系、提示词、流程图源码和当前输出状态，便于下次继续编辑；它不是图片下载入口。

为避免串图，科研图输出只回填当前科研项目签名或当前节点任务匹配的图片，不会把其他话题项目的历史图片自动补到当前画布。
